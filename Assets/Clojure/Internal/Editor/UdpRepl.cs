...
public class UdpReplWindow : EditorWindow {
  ...
  static void StartREPL () {
    RT.load("unity.repl");
    RT.var("unity.repl", "start-udp").invoke();
    // UdpSrvrSample.Main();
...
public class UdpSrvrSample {
    ...
    public static void Main() {
      thread = new Thread(() => UdpSrvrSample.Listen());
      thread.Start();
    }
    ...
    public static void Listen() {
      ...
      UdpClient sock = new UdpClient(new IPEndPoint(IPAddress.Any, 11211));
      IPEndPoint sender = new IPEndPoint(IPAddress.Any, 0);

      Debug.Log("Starting Clojure REPL...");
      RT.load("unity.repl");
      Debug.Log("OK. Listening.");

      while(running) {
        byte[] incomingBytes = sock.Receive(ref sender);
        var code = Encoding.UTF8.GetString(incomingBytes, 0, incomingBytes.Length);

        var result = RT.var("unity.repl", "repl-eval-string").invoke(code);

        byte[] data = Encoding.UTF8.GetBytes(System.Convert.ToString(result));
